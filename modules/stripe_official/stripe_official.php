...
class Stripe_official extends PaymentModule
{
    ...
    public function hookHeader()
    {
        if (in_array($this->context->controller->php_self, array('order', 'order-opc'))
            || Tools::getValue('module') == $this->name && Tools::getValue('controller') == "validation"
            || (Tools::getValue('controller') == "payment" && Tools::getValue('module') == "onepagecheckoutps" && Tools::getValue('pm') == "stripe_official"))
        {
            if (Configuration::get('STRIPE_ENABLE_IDEAL') || Configuration::get('STRIPE_ENABLE_GIROPAY') || Configuration::get('STRIPE_ENABLE_BANCONTACT') || Configuration::get('STRIPE_ENABLE_SOFORT')) {
                $this->context->controller->addJS($this->_path.'/views/js/stripe-push-methods.js');
            }
            ...
            $this->context->controller->addCSS($this->_path.'/views/css/the-modal.css', 'all');
            $this->context->controller->addCSS($this->_path.'/views/css/front.css', 'all');
            $this->context->controller->addJS($this->_path.'/views/js/payment_validation.js');
    ...
    public function hookPayment()
    {
        ...
        {
            $this->context->controller->addJS($this->_path.'/views/js/jquery.the-modal.js');
            $this->context->controller->addCSS($this->_path.'/views/css/the-modal.css');
            ...
            $this->context->smarty->assign(
                array(
                    ...
                    'baseDir' => Tools::getShopDomainSsl(true) . __PS_BASE_URI__,
            ...
            if ($this->context->currency->iso_code == "EUR"
                && (Configuration::get('STRIPE_ENABLE_IDEAL')
                ...
                $this->context->smarty->assign(
                    array(
                        'stripe_order_url' => $this->context->link->getModuleLink('stripe_official', 'validation', array(), true),
    ...
    {
        if (!$this->active || !Configuration::get('PS_SSL_ENABLED'))
            return;

        $this->context->controller->addJS($this->_path.'/views/js/payment_eu.js');

        $payment = $this->hookPayment();

        $payment_options = array(
            'cta_text' => $this->l('Pay by Stripe'),
    ...
    public function displayForm()
    {
        ...
        if (version_compare(_PS_VERSION_, '1.6', '<')) {
            $html = $this->display($this->_path, 'views/templates/admin/configuration15.tpl');
        } else {
