...
<div class="tabs">
  ...
  <div class="sidebar navigation col-md-2">
	{if isset($tab_contents.logo)}
	  <img class="tabs-logo" src="{$tab_contents.logo|escape:'html':'UTF-8'}"/>
	{/if}
	  ...
	  {foreach from=$tab_contents.contents key=tab_nbr item=content}
		<a class="list-group-item migration-tab"
		  ...
		  {if isset($content.icon) && $content.icon != false}
			<i class="{$content.icon|escape:"html":"UTF-8"} pstab-icon"></i>
		  {/if}

		  {$content.name|escape:"html":"UTF-8"}

		  {if isset($content.badge) && $content.badge != false}
			<span class="badge-module-tabs pull-right {$content.badge|escape:"html":"UTF-8"}"></span>
		  {/if}
...
<script type="text/javascript" src="{$new_base_dir|escape:'html':'UTF-8'}views/js/PSTabs.js"></script>
<script type="text/javascript">
		(function() {
