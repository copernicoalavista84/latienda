...
function responsiveResize()
{
	...
	if (($(window).width()+scrollCompensate()) <= 1199 && responsiveflag == false)
	{
		accordion('enable');
...
function display(view)
{
	if (view == 'list')
	{
		...
		$('.product_list .item').each(function(index, element) {
			var html = '';
			html = '<div class="item-content clearfix">';
...
function accordion(status)
{
	if(status == 'enable')
	{
		var accordion_selector = '#left_column .title_block';

		$(accordion_selector).on('click', function(e){
			$(this).toggleClass('active').parent().find('.block_content').stop().slideToggle('medium');
		});
		$('#left_column').addClass('accordion').find('.block_content').slideUp('fast');
		if (typeof(ajaxCart) !== 'undefined')
			ajaxCart.collapse();
	}
	else
	{
		$('#left_column .title_block').removeClass('active').off().parent().find('.block_content').removeAttr('style').slideDown('fast');
