...
<div class="box"><h3 class="subheading">{l s='Follow your order\'s status step-by-step'}</h3>
	...
	<table class="detail_step_by_step table table-bordered">
		...
		{foreach from=$order_history item=state name="orderStates"}
			<tr class="{if $smarty.foreach.orderStates.first}first_item{elseif $smarty.foreach.orderStates.last}last_item{/if} {if $smarty.foreach.orderStates.index % 2}alternate_item{else}item{/if}">
				...
				<td><span{if isset($state.color) && $state.color} style="color:{$state.color|escape:'html':'UTF-8'}; font-weight:700;"{/if} class="label{if isset($state.color) && Tools::getBrightness($state.color) > 128} dark{/if}">{$state.ostate_name|escape:'html':'UTF-8'}</span></td>
			</tr>
		{/foreach}
		</tbody>
	</table>
