    ...
    {foreach from=$multipleAddresses item=address name=myLoop}
        <div class="col-xs-12 col-md-6 address">
            <ul class="{if $smarty.foreach.myLoop.last}last_item{elseif $smarty.foreach.myLoop.first}first_item{/if}{if $smarty.foreach.myLoop.index % 2} alternate_item{else} item{/if} box">
                ...
                <a href="{$link->getPageLink('address', true, null, "id_address={$address.object.id|intval}")|escape:'html':'UTF-8'}" title="{l s='Update'}">{l s='Update'}</a>
                <a href="{$link->getPageLink('address', true, null, "id_address={$address.object.id|intval}&delete")|escape:'html':'UTF-8'}" data-id="addresses_confirm" title="{l s='Delete'}">{l s='Delete'}</a></li>
            </ul>
        </div>
...
<ul class="footer_links clearfix">
	<li><a class="std-btn" href="{$link->getPageLink('my-account', true)|escape:'html':'UTF-8'}"><i class="fa fa-chevron-left left"></i> {l s='Back to your account'}</a></li>
	<li><a class="std-btn" href="{if isset($force_ssl) && $force_ssl}{$base_dir_ssl}{else}{$base_dir}{/if}"><i class="fa fa-chevron-left left"></i> {l s='Home'}</a></li>
</ul>
